...
**Q76（見出しの数値それ自体）.** 「静的値は実測の 200〜1700 倍」——この *実測* は
何の尺度か？
→ ❌ **スケール正規化 `max|Δ|/max|a|` だった**（第62回で発見・修正済）。
`equivalence.compare` が使う正準の相対誤差は **要素ごと** `max(|Δ|/(|a|+1e-12))` で、
こちらで測ると同じ差が 2.6〜7.0 と出て、静的値 3.6e-2 を **上回る**。要素ごとの値は
分母が 0 に近い要素（GELU/LayerNorm の出力）に支配されるので、伝播モデル（典型スケールの
量）と比べる相手としてはスケール正規化が適切である——が、「天井」と一言で呼べば
**要素ごとの意味でも上界だ**と読まれる。→ 両尺度を報告し、どちらと比べているかを
明記した（不変条件 114）。**教訓: 本ラウンドの見出しにも同じ欠陥があった。
自分が今書いた数値こそ、最後にもう一度同じ問いを向ける対象である。**

**Q75.** 模倣に 2 度目の同じ問いを向ける。**フリップを何の意味論で測っているか**？
→ ❌ **argmax を無条件に当てていた**（第62回で発見・修正済）。新視点11 は既に
「argmax を非分類タスクに使うと flip_rate=0 に固まる静かな誤用」を記録し、
`decision.compare_task`（回帰/バイナリ/ランキング/サンプリング）を用意していたのに、
新しく書いた模倣はその層を迂回して `decision_flips` を直接呼んでいた。同じモデル・
同じ発散でも、分類の読みでは 0.0%、回帰の読みでは **94.8%**——回帰モデルは
「フリップ 0%」として出荷される。→ `task=` を通し、非分類は `compare_task` へ委ねる
（不変条件 113）。**教訓: 新しい層を足すとき、既存の層が既に解いた問題を作り直して
いないかを問う。「新しい道具は既存の規律の外にある」と暗に扱うのが、この種の退行の
共通原因である。**

---

### 第62回の総括——4 つの欠陥はすべて *同じ 1 つの問い* から出た

「**この製品が出す数値は、それが主張していることを本当に測っているか**」

1. 静的発散は「予測」と称していたが**許容の天井**だった（Q67・実測の 200〜1700 倍）
2. 「sample 実測」は活性でなく**重み行列**を測っていた（Q72）
3. 「相対発散 0.00e+00」は差が無いのでなく**差が表現できない**設定だった（Q73）
4. 「フリップ率」は**分類の読みでしか意味がなく**、回帰では 0 と 94.8% ほど違った（Q75）
5. 「天井」は**スケール正規化の尺度でしか天井でなく**、要素ごとの相対誤差では
   上回っていた（Q76・**本ラウンドで自分が書いた見出しの数値**）

いずれも「値が間違っている」ではなく「**値が別のものを測っている**」という型である。
第61回までの検査（契約・被覆・目録）は「間違った結論を出さない」を守るが、
「その数が何の数か」は守らない。**測定の意味論は、契約の検査では捕まらない。**
