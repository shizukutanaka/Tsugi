...
### 4. A-5 残: 正規化層の scale リセット（次の担当分）
- **現状**: `propagate()` は LayerNorm/RMSNorm の scale-invariance を amp≈1 と単純化し、
  発散の絶対量リセットをモデル化していない（過大評価＝偽BLOCK 側）。
- **やること**: 減衰係数を *検証してから* 入れる。LayerNorm のヤコビアンは
  `J = (g/σ)(I − 11ᵀ/d − ŷŷᵀ/d)` で 2 方向（平均・半径）を射影落としするため、
  相対発散の増幅は理論上 ≤1 になるはず——これを実 LayerNorm 実装への数値実験で
  確かめる。**確かめる前に係数を入れない**（設計ガードレール 2）。
- **受け入れ基準**: 減衰を入れた版が入れない版を偽OK 方向に倒さないこと
  （発散予測が実測を下回らない）をテストで固定。
