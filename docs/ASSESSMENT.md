...
2. **単一ソース codegen コンパイラ**（Phase 4・要 LLVM/MLIR + 実機・別軸）
...
(2) は環境制約でブロック。手順（`docs/GPU-BRINGUP.md`）と実機入口（`audit_cross_vendor`）は
凍結済みで、実機が来た日に着手できる。**(2) を待つ間に (1) を磨き続けるのは gold-plating の
リスクがある**——「存在すべきでない部品の最適化」（Musk 第 3 段階の戒め）。
