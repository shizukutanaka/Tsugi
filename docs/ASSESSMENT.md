...
プロセス・運用まで含めた評価。2026-07 時点の実状（verify 161/161・CPU 23 スイート
...
2. **検証基盤の厚さと「欠陥→不変条件」文化**: 機械検証可能な不変条件 161 件＋
...
5. **verify.py の成長限界**: 1,400 行超・実行数十秒。テーマ別 12 関数に分割済みだが
   単一ファイルの限界は近い。
6. **ドキュメント肥大**: PERSPECTIVE 15 本＋台帳 3 本。正確だが新規読者の導線が弱い。
...
| P1 | A-12 残: 3 分岐以上の SP fork 一般化（`_computed_fork_merge` の 2 分岐制限緩和） | Opus |
...
| P2 | README からの reading path 整理（新規読者導線） | Sonnet |
