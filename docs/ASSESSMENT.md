...
> まで到達した**。単一ソースが両ベンダーの実機械語になり、その正しさをベンダー自身の
> ツールと LLVM が裏づける。残るのは **実機でしか判らないこと**——生成物を走らせたときの
> 数値・レイアウト接合・性能（L3）だけであり、これは前回の「codegen は不可能」と違って
> *実際に検証不能*である（レイアウト接合には独立オラクルが存在しない: LLVM も行列コアの
