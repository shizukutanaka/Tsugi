...
プロセス・運用まで含めた評価。2026-08 時点の実状（verify 187/187・CPU 23 スイート
...
2. **検証基盤の厚さと「欠陥→不変条件」文化**: 機械検証可能な不変条件 187 件＋
...
187 不変条件で機械検証され、値の taxonomy（数値等価/起動可能/タスク影響/精度ポリシー）は
...
> 完成の一言: **検証器プロダクトは完成した**（使える・正しい・境界が明確）。残るは実機を
> 要する codegen で、それは環境が揃った時点の別プロダクトの仕事。
