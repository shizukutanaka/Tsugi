...
| P1 | A-4 残: L3（生成物の実行・レイアウト接合・数値照合） | 人間（要 GPU） |
...
> *実行だけ*である。第 59 回でこの誤りを正し、codegen は L2（ベンダーのアセンブラが
> 受理）まで到達した。以下の (2) に関する記述はこの追記を前提に読むこと。
...
| **単一ソースが両ベンダーの実機械語まで到達する** | 1 IR → PTX/AMDGCN、ptxas・llvm-mc が受理（不変条件 90-92） |

> 完成の一言: **検証器プロダクトは完成し、codegen も L2 まで到達した**（使える・正しい・
> 配布できる・境界が明確・単一ソースが両ベンダーの実アセンブリになる）。残るのは
> **実機でしか判らないこと**——生成物を走らせたときの数値とレイアウト（L3）だけ。
