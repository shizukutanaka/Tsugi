...
**🔴 問題: 静的発散は実測（スケール正規化）の 200〜1700 倍で、天井を予測として提示していた**
...
**🔴 5 つ目の欠陥（見出しの数値それ自体）: 「天井」は 1 つの尺度でしか天井でない**

同じ問いを **本ラウンドの見出し**に向けた。「静的値は実測の 200〜1700 倍」は、実測を
**スケール正規化** `max|Δ|/max|a|` で測ったときの比である。ところが
`equivalence.compare` が使う正準の相対誤差は **要素ごと** `max(|Δ|/(|a|+1e-12))` で、
こちらで測ると静的値 3.6e-2 を **上回る**:

| クラス | スケール正規化 | 要素ごと | 静的値との関係 |
|---|---|---|---|
...
要素ごとの値は分母が 0 に近い要素（GELU/LayerNorm の出力）に支配されるため、伝播
モデル（op ごとの相対条件数を掛け合わせる *典型スケール* の量）と比べる相手としては
スケール正規化の方が適切である。しかし「天井」と一言で呼べば、要素ごとの意味でも
上界だと読まれる。→ 両尺度を報告し、どちらと比べているかを明記する（不変条件 114）。
本ラウンドで自分が書いた見出しにも同じ欠陥があった、という記録である。
...
束縛できないグラフでは None）。`python verify.py` **215/215**・`python check.py` 全 PASS。
